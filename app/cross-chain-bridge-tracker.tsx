    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent>
        <div className="flex justify-between items-center mb-4">
          <Select value={bridgeType} onValueChange={(value: "sol-avax" | "eth-avax") => setBridgeType(value)}>
          ...
          <>
            <div className="mb-4 text-center">
              <p className="text-lg font-semibold">
                Total for {interval}: {totalAmount.toFixed(2)} {bridgeType === "sol-avax" ? "SOL" : "ETH"}
              </p>
              <p className="text-lg font-semibold">Total USD Value: ${totalUSD.toFixed(2)}</p>
            </div>
            <ChartContainer
              config={{
                amount: {
                  label: `Amount (${bridgeType === "sol-avax" ? "SOL" : "ETH"})`,
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data}>
                  ...
                  <Line yAxisId="left" type="monotone" dataKey="amount" stroke="var(--color-amount)" strokeWidth={2} />
                  <Line
                    yAxisId="right"
                    type="monotone"
                    dataKey="usdValue"
            ...
            <div className="mt-4 flex justify-center space-x-4">
              <div className="flex items-center">
                <Badge className="mr-2" style={{ backgroundColor: "var(--color-amount)" }} />
                <span>{`Amount (${bridgeType === "sol-avax" ? "SOL" : "ETH"})`}</span>
              </div>
              <div className="flex items-center">
                <Badge className="mr-2" style={{ backgroundColor: "var(--color-usdValue)" }} />
                <span>USD Value</span>
              </div>
            </div>
          </>
