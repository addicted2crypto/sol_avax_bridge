...
async function fetchBridgeData() {
  try {
    ...
    const data = await response.json();
    const avaxInflows = data.transfers.filter(
      (t: any) => t.toChain === "Avalanche"
    );
    return avaxInflows;
  } catch (error) {
...
async function fetchRealInflowData(): Promise<InflowData> {
  try {
    ...
    for (const [range, duration] of Object.entries(timeRanges)) {
      ...
      const relevantBridgeTransfers = bridgeData.filter(
      ...
      for (let i = 0; i < intervals; i++) {
        ...
        const intervalBridgeTransfers = relevantBridgeTransfers.filter(
          (t: any) => {
