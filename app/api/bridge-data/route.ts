...
async function fetchDefiLlamaBridgeData() {
  try {
    ...
    const solAvaxTransfers = data.transfers.filter(
      (t: any) => t.fromChain === "Solana" && t.toChain === "Avalanche"
    );
    const ethAvaxTransfers = data.transfers.filter(
      (t: any) => t.fromChain === "Ethereum" && t.toChain === "Avalanche"
    );
...
async function fetchRealBridgeData(): Promise<BridgeData> {
  try {
    const [prices, bridgeData] = await Promise.all([
      fetchBitfinexPrices(),
      fetchDefiLlamaBridgeData(),
    ]);
...
const processTransfers = (
  ...
  for (const [range, duration] of Object.entries(timeRanges) as [
    ...
    const relevantTransfers = transfers.filter(
      (t: any) => new Date(t.timestamp * 1000) > rangeStart
    );
    ...
    for (let i = 0; i < intervals; i++) {
      ...
      const totalAmount = intervalTransfers.reduce(
  ...
  if (!cachedData || currentTime - lastFetchTime > CACHE_DURATION) {
    try {
      ...
      return NextResponse.json(
        { error: "Failed to fetch bridged data" },
        { status: 500 }
      );
    }
  }
